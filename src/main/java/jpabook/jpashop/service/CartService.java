...
import jpabook.jpashop.dto.CartItemDto;
import jpabook.jpashop.dto.CartOrderDto;
...
public class CartService {
    ...
    // 회원 검증
    @Transactional(readOnly = true)
    public boolean validateCartItem(Long cartItemId, String memid){

        Member curMember = memberRepository.findById(memid); // 현재 로그인한 회원 조회
        ...
        if(!StringUtils.equals(curMember.getMemId(), savedMember.getMemId())){ // 로그인한 회원 == 장바구니에 상품을 저장한 회원
            return false;
        }
        return true;
    }
